...
- ViewType: `'dashboard' | 'agents' | 'tasks' | 'projects' | 'project-detail' | 'chat' | 'strategy'`
...
### src/App.tsx
The "brain" of the app. ~530 lines. Contains:
...
**View routing** (lines ~400-530): Renders Sidebar + current view. `project-detail` view uses `selectedProjectId` state.
...
Navigation. Maps `ViewType` values to nav items with icons. Highlights current view. Shows badge counts for agents and tasks.
...
### vite.config.ts
Key settings: `format: 'iife'`, `inlineDynamicImports: true`, `@` alias to `./src`.
...
### goals
- `id` (uuid), `title` (text), `progress` (int 0-100), `status` (text), `owner_agent_id` (uuid), `target_date` (date), `initiatives` (jsonb array), `created_at`
